import environment.frame_runner as fr, math, random, pandas as pd, numpy as np
...
def get_mean_pos(name, timestep):
    df = pd.read_csv('environment/data/test_data.csv')
    try:
        mean_pos = (
...
class GridWorld:
    ...
    def __init__(self, show_graph_every=1, debug_mode=False, means = True):
        ...
        #self.agent_pos = (1,1)
        #self.target_pos = (15,20)
        
        self.agent_spawn_pos = [(5,5), (5,5), (5,10), (10,5), (10,5), (10,10), (10,15)]
        self.target_spawn_pos = [(5,10), (10,5), (5,15), (10,10), (15,5), (10,15), (15,15)]
        
        self.rand = random.randint(0,len(self.agent_spawn_pos)-1)
        self.agent_pos = self.agent_spawn_pos[self.rand]
        self.target_pos = self.target_spawn_pos[self.rand]

        self.timestep = 0
        self.target_achieved = 0
        self.target_missed = 0
        self.n_iteration = 1
        self.transport_timetable = pd.read_csv('environment/data/test_data.csv')
        self.final_reward = 0
    ...
    def step(self, action):
        ...
        self.final_reward = 2000 * (1/(self.timestep+50))
        #self.final_reward = 5
        
        new_state = self.grid
        self.run_frame()
        self.total_reward += self.reward
        return (state, self.reward, self.total_reward, new_state, self.done)
